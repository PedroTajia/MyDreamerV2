...
class RewardModel(nn.Module):
    def __init__(self, stoch_size, deter_size, node_size, activation=nn.SiLU(), type_dist = 'symlog_dist'):
        super().__init__()
        self.type_dist = type_dist
        out_dim = 255 if self.type_dist in ('symlog_dist','symlog_disc') else 1
    ...
    def get_dist(self, dist, std=1.0, min_std=0.1):
        ...
        elif self.type_dist == 'symlog_dist':
            return DiscDist(logits=dist)
...
class ValueModel(nn.Module):
    def __init__(self, stoch_size, deter_size, node_size, activation=nn.SiLU(), type_dist="symlog_dist", num_atoms=255):
        super().__init__()
        out_dim = 1 if type_dist == "normal" else num_atoms
        self.model = nn.Sequential(
            nn.Linear(stoch_size + deter_size, node_size),
            activation,
            nn.RMSNorm(node_size),
